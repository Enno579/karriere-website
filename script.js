...
(function () {
  ...
  // ─── Apply form progressive disclosure ───────────────────────
  function initApplyFormSteps() {
    if (!applyForm) return;

    const stepDrivers = document.getElementById('step-drivers');
    const stepSales = document.getElementById('step-sales');
    const stepSalesDetail = document.getElementById('step-sales-detail');
    const stepHours = document.getElementById('step-hours');
    const durationSelect = applyForm.querySelector('[name="sales_experience_duration"]');
    const activityInput = applyForm.querySelector('[name="sales_experience_activity"]');

    function getRadioValue(name) {
      const checked = applyForm.querySelector(`input[name="${name}"]:checked`);
    ...
    function setFieldsRequired(container, required) {
      ...
      container.querySelectorAll('input, select, textarea').forEach((field) => {
        if (field.type === 'hidden' || field.dataset.optional === 'true') return;
        ...
        if (field.type === 'radio') {
          ...
          if (!requiredRadioNames.has(field.name)) {
            field.required = true;
            requiredRadioNames.add(field.name);
          } else {
            field.required = false;
          }
          return;
    ...
    function setStepVisible(step, visible) {
      if (!step) return;
      step.classList.toggle('form__step--visible', visible);
      step.setAttribute('aria-hidden', visible ? 'false' : 'true');
      setFieldsRequired(step, visible);
    }

    function salesDetailsComplete() {
      const duration = durationSelect ? durationSelect.value : '';
      const activity = activityInput ? activityInput.value.trim() : '';
      return Boolean(duration && activity);
    }

    function updateApplyFormSteps() {
      const drivers = getRadioValue('drivers_license');
      const sales = getRadioValue('sales_experience');

      setStepVisible(stepSales, Boolean(drivers));

      if (!drivers) {
        setStepVisible(stepSalesDetail, false);
        setStepVisible(stepHours, false);
        return;
      }
      ...
      if (sales === 'Ja') {
        setStepVisible(stepSalesDetail, true);
        setStepVisible(stepHours, salesDetailsComplete());
      } else {
        setStepVisible(stepSalesDetail, false);
        setStepVisible(stepHours, true);
      }
    }

    applyForm.querySelectorAll('input[name="drivers_license"], input[name="sales_experience"]').forEach((input) => {
      input.addEventListener('change', updateApplyFormSteps);
    });
    if (durationSelect) durationSelect.addEventListener('change', updateApplyFormSteps);
    if (activityInput) activityInput.addEventListener('input', updateApplyFormSteps);

    applyForm.addEventListener('reset', () => {
      requestAnimationFrame(updateApplyFormSteps);
    });

    updateApplyFormSteps();
  }
